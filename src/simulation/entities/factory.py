...
class EntityFactory:
    ...
    def _add_standard_components(self, entity, entity_id, tag_value):
        ...
        # Add wallet component for workplace
        if hasattr(entity, 'entity_type') and entity.entity_type == EntityType.WORK:
            self.ecs.add_component(
                entity_id,
                "wallet",
                WalletComponent(entity_id, money=1000.0)  # Initial capital
            )

            self.ecs.add_component(
                entity_id,
                "reserves",
                ReservesComponent(entity_id, food=100.0, max_food=1000.0)
            )
