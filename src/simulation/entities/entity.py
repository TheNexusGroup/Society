...
from ...core.assets.animation import Animation
from ...core.assets.manager import AssetManager
    ...
    def update_asset_based_on_state(self, state=None):
        ...
        if hasattr(self, 'is_alive') and not self.is_alive:
            ...
            if "dead" in self.assets:
                asset_name = "dead"
                state = "dead"  # Set state to "dead" for the behavior component
        ...
        for key, asset in self.assets.items():
            asset.visible = key == asset_name
        
        # Make sure the render component is updated with the correct asset
        if hasattr(self, 'ecs_id') and self.ecs_id is not None and hasattr(self, 'world'):
            behavior_component = self.world.ecs.get_component(self.ecs_id, "behavior")
            if behavior_component:
                behavior_component.state = state
                # Also update is_alive in properties
                if hasattr(self, 'is_alive'):
                    behavior_component.properties["is_alive"] = self.is_alive
            
            render_component = self.world.ecs.get_component(self.ecs_id, "render")
            if render_component and asset_name in self.assets:
                render_component.asset = self.assets[asset_name]
                render_component.visible = True
        
        print(f"Updating asset {asset_name} for entity {self.entity_type}")
